...
export function TicketScanner({ onScan, onError }: TicketScannerProps) {
  ...
  const handleError = useCallback((error: any) => {
    ...
    if (error.name === 'NotAllowedError') {
      ...
      // This happens when the video element has zero dimensions, usually temporary
      console.log("Video element size error (temporary):", error.message);
      return;
    } else if (error.message && (
      error.message.includes('initialization') || 
      error.message.includes('Failed to initialize') ||
      error.message.includes('Camera initialization failed') ||
      error.message.includes('getImageData') ||
